import { KMSProvider } from '@eco-foundation/eco-kms-core'
...
export class KMSProviderGCP extends KMSProvider {
  ...
  async getDerPublickey(keyId: string): Promise<Buffer> {
    ...
    const pubKey = await this.kms.getPublicKey({
      name: this.kms.cryptoKeyVersionPath(
        this.path.projectId,
  ...
  async signDigest(keyId: string, digest: Buffer): Promise<Buffer> {
    ...
    if (
      crc32c.calculate(Buffer.from(signResponse.signature)) !==
   ...
   */
  async createKey(
    protectionLevel: 'HSM' | 'SOFTWARE' = 'SOFTWARE'
  ): Promise<string> {
    if (!this.path) {
      throw 'this.path is undefined'
    }
