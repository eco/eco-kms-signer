import { ECDSASignature, Wallets, KMSProvider } from '@eco-foundation/eco-kms-signer/core'
...
export class KMSWallets extends Wallets {
  ...
  public async ecsign(
    ...
    const { r, s } = USignatureECDSA.decodeRS(
      await this.provider.signDigest(account.keyId, digest)
    )

    const address = !account.address
      ? await this.getAddress(account.keyId)
      : account.address
