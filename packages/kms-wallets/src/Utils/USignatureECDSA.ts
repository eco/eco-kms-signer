...
import { UPublickey } from '@eco-foundation/eco-kms-core'
...
export class USignatureECDSA {
  ...
  public static calculateV(
    ...
    if (
      Buffer.compare(
        address,
        UPublickey.fromVRS(digest, candidate_1, r, s, chainId).getAddress()
      ) === 0
    ) {
      return candidate_1
    } else if (
      Buffer.compare(
        address,
        UPublickey.fromVRS(digest, candidate_2, r, s, chainId).getAddress()
      ) === 0
    ) {
      return candidate_2
    } else {
