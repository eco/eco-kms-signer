import { bytesToHex, privateToPublic, ecrecover } from '@ethereumjs/util'
...
export class UPublickey {
  ...
  public getPublickeyHex(): string {
    return bytesToHex(this.getPublickey()).toLowerCase()
  }
  ...
  public static fromPrivatekey(input: Buffer | string) {
    return new UPublickey(
      Buffer.from(privateToPublic(UBuffer.bufferOrHex(input)))
    )
  }
  ...
  public static fromVRS(
    ...
    return new UPublickey(Buffer.from(ecrecover(digest, v, r, s, chainId)))
  }
}
